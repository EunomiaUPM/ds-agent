...
use ymir::errors::{BadFormat, Errors, Outcome, PetitionFailure};
...
use ymir::utils::{expect_from_env, json_headers, trim_4_base, ResponseExt};
...
impl PeerConnectorTrait for GnapPeerConnectorService {
    fn start(
        ...
        let int_model = req_interaction::NewModel {
            id: id.clone(),
            start: vec![InteractStart::Oid4VP.to_string()],
    ...
    async fn send_req(
        ...
        let res = self
            .client
            .post(&int_model.grant_endpoint, Some(headers), body)
            .await?;
        ...
        let res: GrantResponse = if res.status().is_success() {
            ...
            return Err(Errors::provider(
                ...
                error_res
                    .error
                    .unwrap_or("Unexpected error on provider onboarding".to_string()),
                None,
            ));
        };
        ...
        let interact = res.interact.as_ref().ok_or_else(|| {
            Errors::missing_resource("interact ref", "Missing interact reference in db", None)
        })?;
        let cont_data = res
            .r#continue
            .as_ref()
            .ok_or_else(|| Errors::missing_resource("continue", "Missing continue in db", None))?;
    ...
    fn save_verification(
        ...
        let uri = int_model.oidc_vp_uri.as_ref().ok_or_else(|| {
            Errors::missing_resource("oid4vp uri", "Missing oid4vp uri in db", None)
        })?;
