...
impl BffEventFeedRouter {
    ...
    pub async fn handle_list(bus: Arc<EventBus>, q: ListEventsQuery, tenant_id: &str) -> Response {
        let limit = q.limit.unwrap_or(50).min(100);
        let offset = q.offset.unwrap_or(0);

        match bus
            .event_repo()
            .list_events(tenant_id, q.topic.as_deref(), limit, offset)
            .await
        {
            Ok(mut events) => {
    ...
    pub async fn handle_get(bus: Arc<EventBus>, id: String, tenant_id: &str) -> Response {
        ...
        match bus.event_repo().get_event_by_id(tenant_id, &urn).await {
