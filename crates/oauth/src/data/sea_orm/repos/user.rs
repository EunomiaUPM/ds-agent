...
use crate::data::sea_orm::mappers::{user_from_orm, user_to_active_model};
...
impl UserRepository for SeaOrmUserRepository {
    ...
    async fn patch(
        ...
        if let Some(r) = role { active.role = Set(r.to_string()); }
        if let Some(f) = extra_fields { active.extra_fields = Set(f); }
...
fn decode_cursor(cursor: &str) -> Result<chrono::DateTime<chrono::FixedOffset>, ()> {
    let bytes = base64::engine::general_purpose::URL_SAFE_NO_PAD.decode(cursor).map_err(|_| ())?;
