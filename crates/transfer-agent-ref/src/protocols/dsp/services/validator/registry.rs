...
use common::validation::{codes, violation, Validator, ValidatorRegistry, Violations};

use crate::protocols::dsp::entities::context_common::TransferContextProcessSlot;
use crate::protocols::dsp::entities::context_dsp::{TransferDSPContextDomain, TransferDSPContextTyped};
...
impl TransferValidators {
    ...
    pub fn dsp_registry() -> ValidatorRegistry<TransferDSPMessageType, TransferDSPContextDomain> {
        ...
        reg.register(TransferDSPMessageType::TransferCompletionMessage, Self::completion_dsp());
        reg.register(TransferDSPMessageType::TransferSuspensionMessage, Self::suspension_dsp());
        reg.register(TransferDSPMessageType::TransferTerminationMessage, Self::termination_dsp());
        reg
    }
    ...
    pub fn rpc_registry() -> ValidatorRegistry<TransferDSPMessageType, TransferRPCContextDomain> {
        let mut reg = ValidatorRegistry::new();
        reg.register(TransferDSPMessageType::TransferRequestMessage, Self::request_rpc());
        reg.register(TransferDSPMessageType::TransferStartMessage, Self::start_rpc());
        reg.register(TransferDSPMessageType::TransferCompletionMessage, Self::completion_rpc());
        reg.register(TransferDSPMessageType::TransferSuspensionMessage, Self::suspension_rpc());
        reg.register(TransferDSPMessageType::TransferTerminationMessage, Self::termination_rpc());
        reg
    }
    ...
    pub fn edge_registry() -> ValidatorRegistry<TransferDSPMessageType, TransferDSPContextTyped> {
        let mut reg = ValidatorRegistry::new();
        reg.register(TransferDSPMessageType::TransferRequestMessage, Self::request_edge());
        reg.register(TransferDSPMessageType::TransferStartMessage, Self::start_edge());
        reg.register(TransferDSPMessageType::TransferCompletionMessage, Self::completion_edge());
        reg.register(TransferDSPMessageType::TransferSuspensionMessage, Self::suspension_edge());
        reg.register(TransferDSPMessageType::TransferTerminationMessage, Self::termination_edge());
        reg
    }
    ...
    fn request_edge() -> Validator<TransferDSPContextTyped> {
        Validator::new().rule(|ctx: &TransferDSPContextTyped| {
            ...
            if let Some(consumer_pid) = &ctx.fields.consumer_pid {
                ...
                v.push(common::validation::Violation::new("consumerPid", codes::MISSING, "is required"));
            }
            if let Some(agreement_id) = &ctx.fields.agreement_id {
                ...
                v.push(common::validation::Violation::new("agreementId", codes::MISSING, "is required"));
            }
            if ctx.fields.format.as_deref().unwrap_or("").trim().is_empty() {
                v.push(common::validation::Violation::new("format", codes::MISSING, "is required"));
            }
            v.into_result()
        })
    }
    ...
    fn suspension_edge() -> Validator<TransferDSPContextTyped> {
        Validator::new().rule(|ctx: &TransferDSPContextTyped| {
            ...
            if ctx.fields.code.as_deref().unwrap_or("").trim().is_empty() {
                v.push(common::validation::Violation::new("code", codes::MISSING, "is required"));
            }
            v.into_result()
        })
    }
    ...
    fn request_dsp() -> Validator<TransferDSPContextDomain> {
        Validator::new()
            .rule(|ctx: &TransferDSPContextDomain| {
                let mut v = Violations::new();
                Self::check_urn_opt(&mut v, "consumerPid", ctx.typed.fields.consumer_pid.as_deref());
                Self::check_urn_opt(&mut v, "agreementId", ctx.typed.fields.agreement_id.as_deref());
                v.into_result()
            })
            .then()
            .rule(|ctx: &TransferDSPContextDomain| {
                ...
                if let Err(vs) = TransferRules::state_transition(current_state.as_ref(), &ctx.typed.message, "state") {
                    v.extend(vs);
                }
                if let Err(vs) = TransferRules::role_for_message(&ctx.role, &ctx.typed.message, "role") {
                    v.extend(vs);
                }
                v.into_result()
            })
    ...
    fn start_dsp() -> Validator<TransferDSPContextDomain> {
        Validator::new()
            .rule(|ctx: &TransferDSPContextDomain| {
                let mut v = Violations::new();
                Self::check_urn_opt(&mut v, "consumerPid", ctx.typed.fields.consumer_pid.as_deref());
                Self::check_urn_opt(&mut v, "providerPid", ctx.typed.fields.provider_pid.as_deref());
            ...
            .rule(|ctx: &TransferDSPContextDomain| {
                ...
                if let Err(vs) = TransferRules::semaphore(&attr, &ctx.typed.message, &ctx.role, "stateAttribute") {
                    v.extend(vs);
                }
                if let Err(vs) = TransferRules::data_address_format(ctx.typed.fields.data_address.is_some(), &ctx.role, &attr, "dataAddress") {
                    v.extend(vs);
                }
                if let Some(uri_id) = ctx.typed.rdf.parsed.raw.path_id.as_deref() {
                    if let Err(vs) = TransferRules::uri_and_pid(uri_id, ctx.typed.fields.consumer_pid.as_deref(), ctx.typed.fields.provider_pid.as_deref(), &ctx.role, "uriId") {
                        v.extend(vs);
                    }
                }
                let (proc_cons, proc_prov) = Self::extract_stored_pids(&ctx.process);
                if let Err(vs) = TransferRules::correlation(proc_cons, proc_prov, ctx.typed.fields.consumer_pid.as_deref(), ctx.typed.fields.provider_pid.as_deref(), "pids") {
                    v.extend(vs);
                }
                v.into_result()
            })
    ...
    fn suspension_dsp() -> Validator<TransferDSPContextDomain> {
        Validator::new()
            .rule(|ctx: &TransferDSPContextDomain| {
                ...
                Self::check_urn_opt(&mut v, "consumerPid", ctx.typed.fields.consumer_pid.as_deref());
                Self::check_urn_opt(&mut v, "providerPid", ctx.typed.fields.provider_pid.as_deref());
                if ctx.typed.fields.code.as_deref().unwrap_or("").trim().is_empty() {
                    v.push(common::validation::Violation::new("code", codes::MISSING, "is required"));
                }
                v.into_result()
            })
    ...
    fn check_urn_opt(v: &mut Violations, field: &'static str, val: Option<&str>) {
        match val {
            ...
            Some(_) => v.push(common::validation::Violation::new(field, codes::MALFORMED, "must be a valid URN")),
            None => v.push(common::validation::Violation::new(field, codes::MISSING, "is required")),
        }
    }
