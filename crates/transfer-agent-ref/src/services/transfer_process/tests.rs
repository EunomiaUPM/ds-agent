...
async fn get_one_returns_view_with_identifiers() {
    ...
    let svc = make_svc(proc_repo, id_repo);
    let view = svc.get_one(p.id().as_urn()).await.unwrap();
...
async fn get_one_propagates_identifier_repo_error() {
    ...
    let svc = make_svc(proc_repo, id_repo);
    assert!(svc.get_one(p.id().as_urn()).await.is_err());
}
...
async fn batch_empty_ids_returns_empty_vec() {
    ...
    let svc = make_svc(proc_repo, id_repo);
    let views = svc.batch(&BatchRequests { ids: vec![] }).await.unwrap();
...
async fn create_without_identifiers_does_not_call_upsert() {
    ...
    let svc = make_svc(proc_repo, id_repo);
    let view = svc.create(&make_new_cmd(None)).await.unwrap();
...
async fn create_with_one_identifier_upserts_once() {
    ...
    let svc = make_svc(proc_repo, id_repo);
    let view = svc.create(&make_new_cmd(Some(ids))).await.unwrap();
...
async fn create_with_multiple_identifiers_upserts_each() {
    ...
    let svc = make_svc(proc_repo, id_repo);
    let view = svc.create(&make_new_cmd(Some(ids.clone()))).await.unwrap();
...
async fn create_propagates_process_repo_error() {
    ...
    let svc = make_svc(proc_repo, id_repo);
    assert!(svc.create(&make_new_cmd(None)).await.is_err());
}
...
async fn create_propagates_identifier_upsert_error() {
    ...
    let svc = make_svc(proc_repo, id_repo);
    assert!(svc.create(&make_new_cmd(Some(ids))).await.is_err());
}
...
async fn edit_with_state_change() {
    ...
    let svc = make_svc(proc_repo, id_repo);
    let view = svc
        .edit(&urn, &make_edit_cmd(Some("COMPLETED"), None))
        .await
        .unwrap();
...
async fn edit_without_identifiers_skips_upsert() {
    ...
    let svc = make_svc(proc_repo, id_repo);
    svc.edit(&urn, &make_edit_cmd(None, None)).await.unwrap();
}
...
async fn edit_with_identifiers_upserts_then_fetches() {
    ...
    let svc = make_svc(proc_repo, id_repo);
    let view = svc
        .edit(&urn, &make_edit_cmd(None, Some(ids)))
        .await
        .unwrap();
...
async fn edit_propagates_process_repo_error() {
    ...
    let svc = make_svc(proc_repo, id_repo);
    assert!(
        svc.edit(&p_urn(1), &make_edit_cmd(None, None))
            .await
            .is_err()
    );
}
...
async fn edit_propagates_identifier_upsert_error() {
    ...
    let svc = make_svc(proc_repo, id_repo);
    assert!(
        svc.edit(&p_urn(1), &make_edit_cmd(None, Some(ids)))
            .await
            .is_err()
    );
}
