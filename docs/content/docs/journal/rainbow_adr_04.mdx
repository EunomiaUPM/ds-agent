...
We have decided to implement a centralized configuration subsystem within the `common` crate. This system unifies all configuration sources into a "Base File + Environment Override" model. A canonical `config.yaml` file will act as the single source of truth for immutable parameters regarding identity, server binding, and protocol behavior. Any changes to these parameters will require a pod restart, ensuring runtime immutability.
