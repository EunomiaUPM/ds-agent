...
const CatalogItem: React.FC<CatalogItemProps> = ({
  ...
  return (
    <div
      className={`catalog-card bg-background-200/15  hover:bg-background-200/30 transition-all border rounded-md border-white/10 flex flex-col p-4 gap-3 max-w-lg ${conditionalCatalogClasses}`}
    >
        ...
        {id !== null ? (
          <Link to="/catalog/participant/$participantId" params={{ participantId: id }}>
            <Heading level="h3" className="capitalize mb-3 underline-offset-2 hover:underline">
              {title ? title : `${organizationName}'s Catalog for Demo`}
            </Heading>
          </Link>
        ) : (
          <Heading level="h3" className="capitalize mb-3 underline-offset-2 hover:underline">
