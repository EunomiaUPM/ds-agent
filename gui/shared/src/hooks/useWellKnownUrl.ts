...
import {
  ...
  getGetParticipantByIdQueryOptions,
} from "../data/orval/participants/participants";
...
export const useParticipantDSPPath = (participantId: string | undefined, version = "2025-1") => {
  ...
  if (
    participantData &&
    participantData.status === 200 &&
    participantData.data.base_url &&
    participantVersion?.data &&
    "protocolVersions" in participantVersion.data
  ) {
    const { base_url } = participantData.data;
    const protocolVersion = participantVersion.data.protocolVersions.find(
      (p) => p.version === version,
    );
    if (protocolVersion) {
