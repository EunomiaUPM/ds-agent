...
impl ValidationHelpers for ValidationHelperService {
    async fn parse_urn(&self, uri_id: &String) -> Outcome<Urn> {
        Urn::from_str(uri_id.as_str()).map_err(|_| Errors::parse("Invalid URN. The URN is malformed.", None))
    }
    ...
    async fn parse_identifier_into_role(&self, identifier: &str) -> Outcome<RoleConfig> {
        match identifier {
            ...
            _ => {
                return Err(Errors::parse("Not a valid DSP identifiers. Please use 'consumerPid' or 'providerPid'.", None));
            }
        }
    }
    ...
    async fn get_current_dto_from_payload(
        ...
        let consumer_pid = payload.get_consumer_pid().ok_or_else(|| Errors::parse("Not a valid DSP payload, consumer_pid is mandatory.", None))?;
        let dto = self
    ...
    async fn get_current_dto_from_payload_by_provider(
        ...
        let consumer_pid = payload.get_provider_pid().ok_or_else(|| Errors::parse("Not a valid DSP payload, provider_pid is mandatory.", None))?;
        let dto = self
            .negotiation_process_service
            .get_negotiation_process_by_key_value(&consumer_pid)
            .await?
    ...
    async fn get_pid_by_role(
        ...
        let pid = dto.identifiers.get(role_as_identifier).ok_or_else(|| Errors::parse("There is no such a identifier, role is mandatory.", None))?;
    ...
    async fn get_state_from_dto(
        ...
        let state = state.parse::<NegotiationProcessState>().map_err(|_| Errors::parse("Something is wrong. Seems this process' state is not protocol compliant", None))?;
        Ok(state)
    }
