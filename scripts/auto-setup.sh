...
# -----------------------------------------------------------------------------
# Eunomia DS-Agent ENVIRONMENT SETUP SCRIPT
# This script initializes the environment for the Eunomia DS-Agent project. It performs
# the following steps:
...
echo "=== Starting Eunomia DS-Agent environment setup for module '$MODULE'..."
...
if [[ "$MODULE" == "core" ]]; then
  ...
  cargo run --manifest-path "$BASE_DIR/crates/monolith/Cargo.toml" consumer setup \
    --env-file "$BASE_DIR/static/envs/.env.consumer.core"
else
  echo "--- Running setup for Consumer module: $MODULE ---"
  # Targets the specific module's manifest file (e.g., crates/catalog/Cargo.toml)
  cargo run --manifest-path "$BASE_DIR/crates/$MODULE/Cargo.toml" consumer setup \
    --env-file "$BASE_DIR/static/envs/.env.consumer.core"
fi
...
if [[ "$MODULE" == "core" ]]; then
  ...
  cargo run --manifest-path "$BASE_DIR/crates/monolith/Cargo.toml" provider setup \
    --env-file "$BASE_DIR/static/envs/core.consumer.yaml"
else
  echo "--- Running setup for Provider module: $MODULE ---"
  # Targets the specific module's manifest file (e.g., crates/catalog/Cargo.toml)
  cargo run --manifest-path "$BASE_DIR/crates/$MODULE/Cargo.toml" provider setup \
    --env-file "$BASE_DIR/static/envs/core.consumer.yaml"
fi
